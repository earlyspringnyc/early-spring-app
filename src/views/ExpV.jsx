  ...
  return<div>
    ...
    {tab==="budget"&&<div style={{background:"#fff",borderRadius:T.r,padding:48,color:"#111",boxShadow:"0 4px 24px rgba(0,0,0,.4)"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:14,marginBottom:32}}><thead><tr style={{borderBottom:"2px solid #E5E5E5"}}><th style={{textAlign:"left",padding:"10px 0",fontWeight:600,color:"#555",fontSize:11,textTransform:"uppercase",letterSpacing:".06em"}}>Category</th><th style={{textAlign:"right",padding:"10px 0",fontWeight:600,color:"#555",fontSize:11,textTransform:"uppercase",letterSpacing:".06em"}}>Estimated Cost</th></tr></thead>
        <tbody>{cats.map(c=>{const t=ct(c.items).totals;return<React.Fragment key={c.id}><tr style={{borderBottom:"1px solid #F0F0F0",background:"#FAFAF9"}}><td style={{padding:"13px 0",color:"#333",fontWeight:600}}>{c.name}</td><td className="num" style={{padding:"13px 0",textAlign:"right",fontFamily:"monospace",color:"#333",fontWeight:600}}>{f$(t.clientPrice)}</td></tr>
          {c.items.filter(it=>ci(it).clientPrice>0).map(it=><tr key={it.id} style={{borderBottom:"1px solid #F8F8F8"}}><td style={{padding:"8px 0 8px 20px",color:"#555",fontSize:13}}>{it.name}{it.details&&<div style={{fontSize:11,color:"#999",fontStyle:"italic",marginTop:2}}>{it.details}</div>}</td><td className="num" style={{padding:"8px 0",textAlign:"right",fontFamily:"monospace",color:"#555",fontSize:13}}>{f$(ci(it).clientPrice)}</td></tr>)}</React.Fragment>})}
