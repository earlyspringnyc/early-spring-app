...
function ExpV({cats,ag,comp,feeP,project,updateProject,accessToken}){
  ...
  const doSendEmail=async()=>{if(!emailTo.trim())return;if(!accessToken){alert("Google access token required. Sign in with Google OAuth.");return}setEmailSending(true);try{const{sendEmail:gmailSend}=await import('../utils/google.js');const{budgetEmailHtml,timelineEmailHtml}=await import('../utils/emailTemplates.js');let subject,htmlBody;if(tab==="budget"||tab==="proposal"){subject=`Production Estimate: ${project.name||""}`;htmlBody=budgetEmailHtml(project,cats,ag,comp,feeP)}else if(tab==="timeline"){subject=`Project Timeline: ${project.name||""}`;htmlBody=timelineEmailHtml(project,clientTasks)}else{subject=`${project.name||"Project"} — ${tab}`;htmlBody=budgetEmailHtml(project,cats,ag,comp,feeP)}await gmailSend(accessToken,emailTo.trim(),subject,htmlBody);setEmailSent(emailTo);setEmailTo("")}catch(e){alert("Failed to send: "+(e.message||"Unknown error"))}finally{setEmailSending(false)}};
  ...
  return<div>
    ...
    {tab==="budget"&&<div style={{background:"#fff",borderRadius:T.r,padding:48,color:"#111",boxShadow:"0 4px 24px rgba(0,0,0,.4)"}}>
      <div style={{display:"flex",justifyContent:"space-between",paddingBottom:24,marginBottom:32,borderBottom:"2px solid #432D1C"}}>
        <div><div style={{marginBottom:14}}><ESWordmark height={16} color="#432D1C"/></div><div style={{fontSize:28,fontWeight:700,color:"#432D1C"}}>Production Estimate</div><div style={{fontSize:13,color:"#999",marginTop:4,fontFamily:"'Century','Georgia',serif"}}>Prepared by Early Spring</div></div>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:14,marginBottom:32}}><thead><tr style={{borderBottom:"2px solid #E5E5E5"}}><th style={{textAlign:"left",padding:"10px 0",fontWeight:600,color:"#555",fontSize:11,textTransform:"uppercase",letterSpacing:".06em"}}>Category</th><th style={{textAlign:"right",padding:"10px 0",fontWeight:600,color:"#555",fontSize:11,textTransform:"uppercase",letterSpacing:".06em"}}>Estimated Cost</th></tr></thead>
        <tbody>{cats.map(c=>{const t=ct(c.items).totals;return<tr key={c.id} style={{borderBottom:"1px solid #F0F0F0"}}><td style={{padding:"13px 0",color:"#333"}}>{c.name}</td><td className="num" style={{padding:"13px 0",textAlign:"right",fontFamily:"monospace",color:"#333"}}>{f$(t.clientPrice)}</td></tr>})}
          <tr style={{borderTop:"2px solid #432D1C"}}><td style={{padding:"14px 0",fontWeight:700,color:"#432D1C"}}>PRODUCTION SUBTOTAL</td><td className="num" style={{padding:"14px 0",textAlign:"right",fontWeight:700,fontFamily:"monospace",color:"#432D1C"}}>{f$(comp.productionSubtotal.clientPrice)}</td></tr></tbody></table>
    ...
    {tab==="timeline"&&<div>
      ...
      <div style={{background:"#fff",borderRadius:T.r,padding:48,color:"#111",boxShadow:"0 4px 24px rgba(0,0,0,.4)"}}>
        <div style={{display:"flex",justifyContent:"space-between",paddingBottom:24,marginBottom:28,borderBottom:"2px solid #432D1C"}}>
          <div><div style={{marginBottom:14}}><ESWordmark height={16} color="#432D1C"/></div><div style={{fontSize:28,fontWeight:700,color:"#432D1C"}}>Project Timeline</div><div style={{fontSize:13,color:"#999",marginTop:4,fontFamily:"'Century','Georgia',serif"}}>Prepared by Early Spring</div></div>
