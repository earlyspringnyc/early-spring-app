...
function SetV({project,updateProject,onDelete,user}){
  ...
  const F=({label,field})=><div style={{marginBottom:18}}><label style={{display:"block",fontSize:10,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".08em",marginBottom:7}}>{label}</label>
    <input value={project[field]||""} onChange={e=>updateProject({[field]:e.target.value})} style={{width:"100%",padding:"10px 14px",borderRadius:T.rS,background:T.surface,border:`1px solid ${T.border}`,color:T.cream,fontSize:14,fontFamily:T.sans,outline:"none"}}/></div>;
  ...
  return<div style={{maxWidth:isAdmin?700:500}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}>
      <div><h1 style={{fontSize:24,fontWeight:600,color:T.cream,letterSpacing:"-0.02em"}}>Settings</h1><p style={{fontSize:13,color:T.dim,marginTop:6,fontFamily:T.serif,fontStyle:"italic"}}>Changes save automatically</p></div>
    </div>
    ...
    {isAdmin&&<Card style={{padding:28,marginBottom:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <div style={{fontSize:13,fontWeight:600,color:T.cream}}>Team & Permissions</div>
      ...
      <p style={{fontSize:10,color:T.dim,marginTop:12,fontFamily:T.serif,fontStyle:"italic"}}>Team members sign in with Google. Permissions control which sections they can access.</p>
    </Card>}
    <Card style={{padding:28}}><div style={{fontSize:13,fontWeight:600,color:T.neg,marginBottom:12}}>Danger Zone</div>
