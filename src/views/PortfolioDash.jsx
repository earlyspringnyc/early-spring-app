    ...
    <div className="portfolio-container" style={{maxWidth:1100,margin:"0 auto",padding:"40px 32px"}}>
      ...
      {tab==="dashboard"&&<div className="fade-up">
        ...
        <Card style={{overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr .5fr",padding:"12px 18px",borderBottom:`1px solid ${T.border}`,background:T.surface}}>{["Project","Grand Total","Net Profit","Margin","Docs"].map((h,i)=><span key={i} style={{fontSize:10,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".1em",textAlign:i>0?"right":"left"}}>{h}</span>)}</div>
                ...
                {stageProjects.map(p=>{const comp=calcProject(p);const hasData=comp.productionSubtotal.actualCost>0;const ov=(p.docs||[]).filter(d=>d.status==="overdue"||(d.status==="pending"&&isOverdue(d))).length;
                  return<div key={p.id} draggable onDragStart={e=>{setDragProjectId(p.id);e.dataTransfer.effectAllowed="move"}} onDragEnd={()=>{setDragProjectId(null);setDropStage(null)}}><Card hoverable onClick={()=>onOpen(p.id)} style={{padding:0,overflow:"hidden",opacity:stage==="archived"?.6:dragProjectId===p.id?.4:1,cursor:"grab"}}>
                    <div style={{padding:"22px 24px 18px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
                          ...
                          <div style={{flex:1,minWidth:0}}><h3 style={{fontSize:14,fontWeight:600,color:T.cream,marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</h3><p style={{fontSize:12,color:T.dim}}>{p.client||"No client"}</p></div></div><div style={{display:"flex",gap:4,alignItems:"center"}}>{ov>0&&<span style={{fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:10,background:"rgba(248,113,113,.12)",color:T.neg}}>{ov} overdue</span>}<span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:6,background:`${STAGE_COLORS[stage]}15`,color:STAGE_COLORS[stage],textTransform:"uppercase"}}>{STAGE_LABELS[stage]}</span></div></div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:16}}><div><div style={{fontSize:10,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".06em",marginBottom:4}}>Grand Total</div><div className="num" style={{fontSize:18,fontWeight:700,color:T.gold,fontFamily:T.mono}}>{f0(comp.grandTotal)}</div></div><div><div style={{fontSize:10,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".06em",marginBottom:4}}>Net Profit</div><div className="num" style={{fontSize:18,fontWeight:700,color:comp.netProfit>0?T.pos:T.dim,fontFamily:T.mono}}>{f0(comp.netProfit)}</div></div></div></div>
