  ...
  return<div>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center",marginTop:24,flexWrap:"wrap"}}>
      ...
      <div style={{position:"relative"}}>
        ...
        {showExportMenu&&<div className="fc-panel" style={{position:"absolute",bottom:"100%",left:0,marginBottom:6,zIndex:20,minWidth:200,padding:4,borderRadius:12}}>
          {[["Google Sheets",exportToSheets,"Opens in Google Sheets"],["XLSX",()=>{exportXLS();setShowExportMenu(false)},"Spreadsheet"],["CSV",exportCSV,"Comma-separated"],["PDF",exportPDF,"Print to PDF"]].map(([label,fn,sub],i,arr)=>
    ...
    {canEdit&&<div style={{marginTop:32}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        ...
        <button onClick={saveSnapshot} className="btn-pill" style={{padding:"6px 14px",fontSize:11}}>Save current version</button>
      </div>
      {showHistory&&<div style={{display:"flex",flexDirection:"column",gap:0,borderTop:`1px solid ${T.ink}`}}>
        {history.length===0&&<div style={{padding:24,textAlign:"center",color:T.fadedInk,fontSize:12,borderBottom:`1px solid ${T.faintRule}`}}>No saved versions yet.</div>}
        {history.slice().reverse().map(h=><div key={h.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0",borderBottom:`1px solid ${T.faintRule}`}}>
          <div>
            <div style={{fontSize:14,fontWeight:600,color:T.ink}}>{h.label}</div>
            <div style={{fontSize:11,color:T.fadedInk,marginTop:3,fontFamily:T.mono}}>{new Date(h.date).toLocaleDateString()} · {new Date(h.date).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>restoreSnapshot(h)} className="btn-pill" style={{padding:"5px 14px",fontSize:10}}>Restore</button>
