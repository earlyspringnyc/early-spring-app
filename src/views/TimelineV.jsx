...
function TimelineV({project,updateProject,canEdit,accessToken,requestCalendarAccess}){
  ...
  <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:12,marginBottom:12}}>
    ...
    <div><label style={{display:"block",fontSize:9,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".08em",marginBottom:5}}>Time</label>{(()=>{const[customTime,setCustomTime]=useState(false);const times=[];for(let h=7;h<=21;h++){times.push(`${String(h).padStart(2,"0")}:00`);times.push(`${String(h).padStart(2,"0")}:30`)}if(customTime)return<input autoFocus value={meetingTime} onChange={e=>setMeetingTime(e.target.value)} onBlur={()=>{if(!meetingTime)setCustomTime(false)}} placeholder="14:00" style={{width:"100%",padding:"9px 12px",borderRadius:T.rS,background:T.surface,border:`1px solid ${T.cyan}`,color:T.cream,fontSize:13,fontFamily:T.mono,outline:"none"}}/>;return<select value={meetingTime||""} onChange={e=>{if(e.target.value==="__custom__"){setCustomTime(true);setMeetingTime("")}else setMeetingTime(e.target.value)}} style={{width:"100%",padding:"9px 8px",borderRadius:T.rS,background:T.surface,border:`1px solid ${T.border}`,color:meetingTime?T.cream:T.dim,fontSize:13,fontFamily:T.mono,outline:"none",appearance:"none",WebkitAppearance:"none",cursor:"pointer"}}><option value="">Select time</option>{times.map(t=><option key={t} value={t}>{t.startsWith("0")?t.slice(1):t}{parseInt(t)<12?" AM":parseInt(t)===12?" PM":" PM"}</option>)}<option value="__custom__">Custom time…</option></select>})()}</div>
  ...
  <div style={{marginBottom:12,position:"relative"}}><label style={{display:"block",fontSize:9,fontWeight:600,color:T.dim,textTransform:"uppercase",letterSpacing:".08em",marginBottom:5}}>Attendees</label>
    {(()=>{const[suggestions,setSuggestions]=useState([]);const[showSug,setShowSug]=useState(false);const attendeeList=meetingAttendees?meetingAttendees.split(",").map(s=>s.trim()).filter(Boolean):[];
    const onInput=async(e)=>{setMeetingAttendees(e.target.value);const parts=e.target.value.split(",");const current=parts[parts.length-1].trim();if(current.length>=2&&accessToken){const results=await searchContacts(accessToken,current);setSuggestions(results);setShowSug(results.length>0)}else{setShowSug(false)}};
    const pickSuggestion=(email)=>{const parts=meetingAttendees.split(",");parts[parts.length-1]=email;setMeetingAttendees(parts.join(", ")+", ");setShowSug(false)};
    return<div>
      ...
      <input value={meetingAttendees} onChange={onInput} onFocus={()=>{if(suggestions.length)setShowSug(true)}} onBlur={()=>setTimeout(()=>setShowSug(false),200)} placeholder="Start typing a name or email..." style={{width:"100%",padding:"9px 12px",borderRadius:T.rS,background:T.surface,border:`1px solid ${T.border}`,color:T.cream,fontSize:12,fontFamily:T.sans,outline:"none"}}/>
      {showSug&&<div style={{position:"absolute",left:0,right:0,top:"100%",zIndex:50,background:"rgba(12,10,20,.97)",border:`1px solid ${T.border}`,borderRadius:T.rS,boxShadow:"0 8px 24px rgba(0,0,0,.4)",maxHeight:160,overflow:"auto"}}>
        {suggestions.map((c,i)=><button key={i} onMouseDown={e=>{e.preventDefault();pickSuggestion(c.email)}} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"transparent",border:"none",borderBottom:`1px solid ${T.border}`,cursor:"pointer",textAlign:"left",fontSize:11,color:T.cream,fontFamily:T.sans}} onMouseEnter={e=>e.currentTarget.style.background=T.surfHov} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
