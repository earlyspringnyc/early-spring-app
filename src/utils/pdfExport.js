...
// ─────────────────────────────────────────────────────────────────────
const STATUS_RGB = {
  'not started': { fill: [245, 247, 250], fg: [107, 116, 128], border: [205, 215, 235] },
  'on track':    { fill: [231, 238, 250], fg: [15, 82, 186],   border: [180, 200, 235] },
  'at risk':     { fill: [253, 243, 220], fg: [165, 111, 0],   border: [240, 184, 73] },
  'delayed':     { fill: [248, 224, 224], fg: [122, 31, 31],   border: [220, 170, 170] },
  'complete':    { fill: [15, 82, 186],   fg: [255, 255, 255], border: [15, 82, 186] },
  'on hold':     { fill: [245, 247, 250], fg: [107, 116, 128], border: [180, 195, 215] },
};

function parseDateMDY(mmddyyyy) {
...
export async function exportWorkbackPDF(project, items, opts = {}) {
  const rows = Array.isArray(items) ? items : [];
  const filename = opts.filename || `${project?.name || 'workback'}-workback-schedule.pdf`;
  const doc = new jsPDF({ unit: 'pt', format: 'letter', orientation: 'landscape' });
  const margin = 40;
  const lockup = await getEsLockupPng(96);

  const eventDateStr = project?.eventDate || project?.data?.eventDate || '';
  const eventDate = parseDateMDY(eventDateStr);
  drawTitle(doc, {
    margin,
  ...
  const body = rows.map(r => {
    ...
    const titleLines = [r.name || ''];
    if (r.annotation) titleLines.push(r.annotation);
    if (r.intro) titleLines.push(r.intro);
    (r.items || []).forEach(b => { if (b && b.trim()) titleLines.push(`•  ${b.trim()}`); });
    const milestoneContent = titleLines.join('\n');
    ...
    return [
      ...
      { content: status, styles: { fillColor: sc.fill, textColor: sc.fg, fontStyle: 'bold', fontSize: 8, halign: 'center', valign: 'top' } },
      { content: tDay, styles: { fontSize: 8, halign: 'right', valign: 'top', textColor: FADED } },
    ];
  });
  ...
  autoTable(doc, {
    ...
    head: [[
      ...
      { content: 'T-Day',     styles: { halign: 'right' } },
    ]],
    body,
    columnStyles: {
      0: { cellWidth: 95 },
