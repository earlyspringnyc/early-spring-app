...
function AddSectionModal({ onClose, onAdd }) {
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,.6)", backdropFilter: "blur(8px)" }}>
    <div className="slide-in modal-inner" style={{ width: 400, padding: 32, borderRadius: T.r, background: "rgba(12,10,20,.95)", border: `1px solid ${T.border}`, boxShadow: "0 24px 80px rgba(0,0,0,.5)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><h2 style={{ fontSize: 18, fontWeight: 600, color: T.cream }}>New Budget Section</h2><button onClick={onClose} style={{ background: "none", border: "none", color: T.dim, fontSize: 20, cursor: "pointer", padding: 4, lineHeight: 1 }}>×</button></div>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button onClick={submit} style={{ flex: 1, padding: 11, borderRadius: T.rS, border: "none", background: name.trim() ? `linear-gradient(135deg,${T.gold},#E8D080)` : "rgba(255,255,255,.05)", color: name.trim() ? T.brown : "rgba(255,255,255,.2)", fontSize: 13, fontWeight: 700, cursor: name.trim() ? "pointer" : "default", fontFamily: T.sans }}>Add Section</button>
      </div>
    </div>
  </div>;
}
