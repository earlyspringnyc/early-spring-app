...
import { BackI } from '../icons/index.js';

function Side({view,setView,comp,user,project,onBack,toggleTheme,themeMode}){
  ...
  return<div className="desktop-side" style={{width:228,display:"flex",flexDirection:"column",fontFamily:T.sans,background:"linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.005))",borderRight:`1px solid ${T.border}`}}>
    ...
    <div style={{padding:"10px 22px 18px"}}><div style={{display:"flex",alignItems:"center",gap:7}}><ESIsotype size={18} color={T.gold}/><span style={{fontSize:10,fontWeight:600,letterSpacing:".14em",color:T.gold}}>EARLY SPRING</span></div><h2 style={{fontSize:14,fontWeight:600,color:T.cream,marginTop:8,lineHeight:1.3}}>{project.name}</h2><p style={{fontSize:11,color:T.dim,marginTop:2}}>{project.client||"No client"}</p></div>
    ...
    <nav style={{flex:1,padding:"0 10px",overflow:"auto"}}>
      ...
      <div style={{margin:"10px 4px 6px",padding:"0 10px"}}><span style={{fontSize:9,fontWeight:700,color:T.dim,textTransform:"uppercase",letterSpacing:".12em"}}>Toolbox</span></div>
      <div style={{margin:"0 4px 4px",padding:"6px",borderRadius:T.rS,background:"rgba(255,255,255,.015)",border:`1px solid ${T.border}`}}>
