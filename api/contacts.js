...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!fallback.ok) return res.status(200).json({ contacts: [] });
      const data = await fallback.json();
      const contacts = (data.results || [])
        .filter(r => r.person?.emailAddresses?.length)
        .map(r => ({ name: r.person.names?.[0]?.displayName || '', email: r.person.emailAddresses[0].value }));
    ...
    const contacts = people
      ...
      .map(p => ({ name: p.names?.[0]?.displayName || '', email: p.emailAddresses[0].value }));
